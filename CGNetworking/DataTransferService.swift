...
public protocol DataTransferService {
    ...
    @discardableResult
    func multipartRequest<T: Decodable, E: ResponseRequestable>(with endpoint: E,
                                                                completion: @escaping CompletionHandler<T>) -> NetworkCancellable? where E.Response == T
}
...
// MARK:- Default Data Transfer Service
...
extension DefaultDataTransferService: DataTransferService {
    ...
    public func multipartRequest<T: Decodable, E: ResponseRequestable>(with endpoint: E,
                                                                       completion: @escaping CompletionHandler<T>) -> NetworkCancellable? where E.Response == T {
        return self.networkService.multipartRequest(endpoint: endpoint) { result in
            switch result {
            case .success(let data):
                let result: Result<T, DataTransferError> = self.decode(data: data, decoder: endpoint.responseDecoder)
                DispatchQueue.main.async { return completion(result) }
